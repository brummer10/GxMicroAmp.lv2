...
#define GTK_MICROAMP_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_MICROAMP_KNOB, GtkMicroAmpKnobPrivate))

G_DEFINE_TYPE(GtkMicroAmpKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkMicroAmpKnobPrivate GtkMicroAmpKnobPrivate;

struct _GtkMicroAmpKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkMicroAmpKnobPrivate *priv = GTK_MICROAMP_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkMicroAmpKnob *knob = GTK_MICROAMP_KNOB(widget);
        ...
        if (rect_width > rect_height) {
            offset = (rect_width - rect_height)/2;
            rect_width = rect_height;
            x0    += offset;
        } else {
            offset = (rect_height - rect_width)/2;
        ...
        if (ne_w != rect_width*rect_height || !(GDK_IS_PIXBUF (knob->pswitch_image))
            ...
            if (!sw_state) {
                knob->pswitch_image = gdk_pixbuf_scale_simple(
                  knob->switch_off_image, rect_width ,rect_height, GDK_INTERP_NEAREST);
                //g_object_unref(knob->switch_on_image);
            } else {
                knob->pswitch_image = gdk_pixbuf_scale_simple(
                  knob->switch_on_image, rect_width ,rect_height, GDK_INTERP_NEAREST);
...
{
    g_assert(GTK_IS_MICROAMP_KNOB(widget));
    GtkMicroAmpKnob *knob = GTK_MICROAMP_KNOB(widget);
    GtkMicroAmpKnobPrivate *priv = GTK_MICROAMP_KNOB_GET_PRIVATE(knob);
...
{
    GtkMicroAmpKnob *knob = GTK_MICROAMP_KNOB(object);
...
/****************************************************************
 ** init the GtkMicroAmpKnobClass
 */

static void gtk_knob_class_init (GtkMicroAmpKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkMicroAmpKnobPrivate));
}
...
static void gtk_knob_init (GtkMicroAmpKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_MICROAMP_KNOB, NULL ));
