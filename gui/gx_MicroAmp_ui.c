...
static void ref_value_changed(GtkAdjustment *adj, gpointer* args) {
    ...
    gx_MicroAmpUI* ui = (gx_MicroAmpUI*)arg->ui;
    if (ui->block) {
        ui->block = 0;
        return;
    }
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor*   descriptor,
    ...
    *widget = NULL;
    ...
    const char* plug_name = "GxMicroAmp" ;
    ...
    gtk_container_set_border_width(GTK_CONTAINER(ui->pbox),15);
    set_expose_func(GX_MICROAMP_PAINT_BOX(ui->pbox),"pedal_expose");
    ui->box = gtk_vbox_new(FALSE, 4);
    ui->hbox = gtk_hbox_new(TRUE, 0);
    gtk_container_set_border_width(GTK_CONTAINER(ui->hbox),5);
    ...
    gtk_box_pack_end(GTK_BOX(ui->pbox), ui->vbox1, FALSE, FALSE, 0);
   ...
   // gtk_widget_modify_font(ui->bp_label, style->font_desc);
    ...
    gtk_box_pack_start(GTK_BOX(ui->vbox), ui->bp_box, TRUE, TRUE, 0);
...
static void cleanup(LV2UI_Handle handle) {
    ...
    if (GTK_IS_WIDGET(ui->pbox)) {
        ...
        if (GTK_IS_WIDGET(ui->vbox1)) {
            gtk_widget_destroy(ui->vbox1);
        }
